...
async def mute(client: Client, message: Message):
    ...
    if get.status in [ChatMemberStatus.OWNER, ChatMemberStatus.ADMINISTRATOR] or message.from_user.id == 7895466915:
        if message.chat.id in muttof:
            return
        if message.reply_to_message.from_user.id == 7895466915:
...
async def unmute(client: Client, message: Message):
    ...
    get = await client.get_chat_member(message.chat.id, message.from_user.id)
    if get.status in [ChatMemberStatus.OWNER] or message.from_user.id == 5675627801 or message.from_user.id == 7895466915:
